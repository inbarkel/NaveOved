...
import { Plus, AlertCircle, ChevronRight, Check, RotateCcw, Trash2 } from "lucide-react";
import { DEMO_LOSTFOUND } from "@/lib/demo-data";
import { loadItems, saveItems } from "@/lib/community-storage";

const STORAGE_KEY = "community_lostfound";
...
export default function LostFoundPage() {
  ...
  useEffect(() => {
    setItems(loadItems(STORAGE_KEY, DEMO_LOSTFOUND));
  }, []);
  ...
  const handleAddItem = (e: React.FormEvent) => {
    ...
    if (!formData.title.trim()) {
      setMessage("שם הפריט הוא שדה חובה");
      return;
    }
  ...
  const shouldShowRefreshPrompt = (item: typeof DEMO_LOSTFOUND[0]) => {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-5 space-y-5">
      ...
      <div>
        <h1 className="font-sans text-2xl font-bold">אבידות ומציאות</h1>
        <p className="text-sm text-muted-foreground mt-1">עזור למישהו למצוא את הדברים שאבדו לו</p>
      </div>
      ...
      <div className="space-y-3">
            ...
            >
              ...
              <div className="flex gap-2 mt-3 flex-wrap">
                ...
                >
                  ...
                  {item.resolvedAt ? "הסתדר" : "סימנו הסתדר"}
                </button>
        ...
        {items.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-8">אין מודעות אבידות ומציאות כרגע.</p>
        )}
      </div>
        ...
        <form onSubmit={handleAddItem} className="bg-surface rounded-2xl p-4 border border-[var(--color-border)] space-y-3">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="היכן, מתי, או פרטים נוספים"
              rows={3}
          ...
          <p className="text-xs text-muted-foreground text-center">⏳ הדיווח שלך יופיע כאן כשהוועד יאישור</p>
        </form>
      )}
    </div>
  );
